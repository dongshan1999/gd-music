...
func get_playback_state():
	return get_app_state().playback_state

## 返回播放队列曲目 ID 数组引用。
func get_playback_track_ids_ref() -> Array[String]:
	return get_playback_state().track_ids
...
func get_selected_playlist_index() -> int:
	return get_playback_state().selected_playlist_index

## 设置当前选中的歌单索引。
func set_selected_playlist_index(value: int) -> void:
	get_playback_state().selected_playlist_index = value

## 根据曲目 ID 获取曲目对象。
func get_track_by_id(track_id: String) -> TrackData:
	return get_app_state().get_track_by_id(track_id)

## 返回当前选中的曲目 ID。
func get_selected_track_id() -> String:
	return get_playback_state().selected_track_id

## 设置当前选中的曲目 ID。
func set_selected_track_id(value: String) -> void:
	get_playback_state().selected_track_id = value.strip_edges()

## 返回播放队列当前游标。
func get_playback_queue_index() -> int:
	return get_playback_state().queue_index

## 设置播放队列当前游标。
func set_playback_queue_index(value: int) -> void:
	get_playback_state().queue_index = value

## 返回当前全局播放模式。
func get_playback_mode() -> int:
	return get_playback_state().mode

## 设置当前全局播放模式。
func set_playback_mode(value: int) -> void:
	get_playback_state().mode = value

## 返回当前累计播放秒数。
func get_elapsed_seconds() -> int:
	return get_playback_state().elapsed_seconds

## 设置当前累计播放秒数。
func set_elapsed_seconds(value: int) -> void:
	get_playback_state().elapsed_seconds = value

## 返回当前是否处于播放态。
func is_playing() -> bool:
	return get_playback_state().is_playing

## 设置当前播放态标记。
func set_is_playing(value: bool) -> void:
	get_playback_state().is_playing = value
...
func normalize_favorite_playlist() -> void:
	...
	if playlist_state_controller != null:
		playlist_state_controller.normalize_favorite_playlist()
...
func is_track_liked(track: TrackData) -> bool:
	...
	if playlist_state_controller == null:
		return false
	return playlist_state_controller.is_track_liked(track)
...
func toggle_track_liked(track: TrackData) -> bool:
	...
	if playlist_state_controller == null:
		return false
	return playlist_state_controller.toggle_track_liked(track)
...
func get_current_track():
	return get_track_by_id(get_selected_track_id())
